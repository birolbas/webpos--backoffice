...
function AddRecipe() {
    ...
    const deleteRecipeFromDB = async () => {
        ...
        const response = await fetch("http://localhost:5000/deleteRecipe", {
            ...
            body: JSON.stringify({
                id: recipeToDelete[deleteUiIndex].id,
    ...
    useEffect(() => {
        console.log("sta", staticRecipes)
    }, [staticRecipes])
     
    function filterByName(nameinput) {
        var filtered = []
        console.log("static", staticRecipes)
        filtered = staticRecipes.filter(item =>
        ...
        {isSureToDelete && (
            <div className={staticStyles["confirm-container"]}>
                ...
                <div className={staticStyles["confirm-buttons"]}>
                    ...
                    <button onClick={() => deleteRecipeFromDB()} style={{ backgroundColor: "green", color: "white" }}>Reçeteyi Sil</button>
                </div>
            </div>
        )}
        {recipeTypeInput ? (
