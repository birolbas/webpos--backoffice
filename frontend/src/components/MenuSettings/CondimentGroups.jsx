...
function CondimentGroups() {
    ...
    const [condimentItemCategories, setCondimentItemCategories] = useState([])
    ...
    const getFromDB = async () => {
        ...
        const groupedCondimentItems = data.reduce((acc, item) => {
        ...
        setCondimentItemCategories(Object.keys(groupedCondimentItems))
        setCondimentItems(groupedCondimentItems)
        const prevCondimentalsResponse = await fetch("http://localhost:5000/getCondimentGroups")
        const prevCondimentals = await prevCondimentalsResponse.json()
        setSavedCondimentGroups(prevCondimentals)
    }
    ...
    return <div className={staticStyles["content-container"]}>
        <div className={styles["append-menu-item"]}>
            ...
            <div className={staticStyles["save-button"]}>
                <button onClick={() => saveToDB()} >Yeni Reçete</button>
            </div>
        </div>
        <div className={styles["page-split"]} >
            <div className={staticStyles["table-container"]}>
                ...
                <div className={styles["name-input"]}>
                    ...
                    <input onChange={(e) => setCondimentGroupName(e.target.value)} type="text" />
                </div>
                <div>
                            ...
                            <div key={`categoryIndex ${categoryIndex}`} className={styles["choice-group"]}>
                                <div className={styles["group-infos"]}>
                                    <div className={styles["name-input"]}>
                                        ...
                                        <input onChange={(e) => changeCondimentGroupName(e.target.value, categoryIndex)} style={{ width: "20%", height: "1rem" }} type="text" name="" id="" />
                                    </div>
                                    ...
                                    {category.items.length > 0 ?
                                        category.items.map((item, _) => (
                                            <div className={styles["item-in-group"]}>
                                                ...
                                                <div className={styles["delete-item-in-group"]}>
                                ...
                                {group[1].map((item, itemIndex) => (
                                    <div data-itemGroupIndex={group[0]} data-itemIndex={itemIndex} draggable="true" onDragStart={handleDragStart} className={styles["group-item"]}>
                                        ...
                                        <div>
                                            ...
                                            <p className={styles["group-items-price"]}>{item.price.toFixed(2)} </p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                ...
                <div className={styles["saved-condiment-group"]}>
                    <div className={styles["saved-condiment-group-headers"]}>
                        ...
                        <div className={styles["delete-edit-saved-condiment"]}>
                            <svg style={{color:"aquamarine"}} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
